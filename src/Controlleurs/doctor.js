...
// Créer un agent HTTPS qui désactive la vérification SSL
const httpsAgent = new https.Agent({
  rejectUnauthorized: false, // Ignore la vérification du certificat SSL
});



// Exemple d'appel de la fonction
const from = 'sender@vats';
const to = '98506504';  // Numéro de téléphone du destinataire
const content = 'Hello 1';  // Contenu du message
...
const getDoctorsById = async (req, res) => {
    const doctorId = req.query.doctor_id;
    let duration = parseInt(req.query.duration, 10); // Durée en jours envoyée par le frontend
    const nextDays = parseInt(req.query.nextDays, 10); // Indicateur si "suivant" est cliqué
    ...
    // Définir une durée par défaut de 3 jours si aucune durée n'est donnée
    if (!duration || isNaN(duration) || duration <= 0) {
        duration = 3;
    }

    // Ajouter des jours supplémentaires si le frontend demande des jours suivants
    if (nextDays) {
        duration = duration + nextDays; // Ajouter la durée des jours supplémentaires
    }
    ...
    try {
        ...
        // Fonction pour obtenir le jour de la semaine en anglais
        const getDayOfWeekInEnglish = (date) => {
            const options = { weekday: 'long' };
            return date.toLocaleString('en-US', options).toLowerCase(); // Utiliser en-US pour obtenir le jour en anglais
        };
        ...
        const generateWeeklySchedule = (availabilities, startDate, daysCount) => {
            ...
            for (let i = 0; i < daysCount; i++) {
                ...
                const dayOfWeek = getDayOfWeekInEnglish(currentDate); // Utiliser la version anglaise du jour de la semaine
        ...
        // Retourner la réponse avec les horaires
        res.json({
            schedule,
        });
    } catch (err) {
